...
// переключатели для входных данных
...
// светодиоды для вывода результата
...
// светодиоды для вывода количества тестирования
...
// кнопка для активации режима просто подсчёта sqrt3 ( кубического корня )
set_property PACKAGE_PIN P17 [get_ports sqrt3_button_status]

// кнопка для активации режима тестирования
set_property PACKAGE_PIN M17 [get_ports testing_button_status]

// эта кнопка обозначает - надо ли брать то-что в input_data - как первое число для генератора чисел LSFR
set_property PACKAGE_PIN T8 [get_ports get_lsfr_base_from_data]

// кнопка для reset
set_property PACKAGE_PIN P18 [get_ports rst]

// светодиод ( если он горит - значит что плата ещё думает и ответ не готов, иначе если он негорит, то ответ готов )
set_property PACKAGE_PIN N16 [get_ports busy]

// для генерации clk - тактового сигнала
